...
默认只安装核心依赖，不会下载大型向量模型。需要额外功能时再选择：

```bash
python install.py --vector   # 语义向量记忆
python install.py --wechat   # 微信接入
```
...
浏览器打开 `http://127.0.0.1:8000`：流式对话、图片上传、语音发送、以及随时调整参数（温度/长度/节奏等，改完即时生效）。

网页端支持停止生成、重新生成和复制消息。设置页的“数据与隐私”区域可以导出完整备份，也可以选择备份并安排在下次启动时安全恢复。
