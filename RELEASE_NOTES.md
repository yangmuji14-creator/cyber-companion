...
- Web 端新增「发现」朋友圈：发布/点赞/评论/删除 + AI 角色自动发布（MomentsAutoPoster，LLM 或模板生成）
- 新增「通讯录」角色花名册；导航分区对齐 聊天/通讯录/发现/记忆/设置
- MCP 扩展完整接入 Web 设置：server 新增/编辑/JSON 导入、测试/连接/刷新/工具查看
- 新增「插件管理」目录页（内置 + MCP 工具）
- 新增「AI 语音回复」（TTS）：OpenAI 兼容 /v1/audio/speech，服务商 CRUD + 试听，回复用 [语音] 标记合成并渲染可播放气泡
- 设置主页改为卡片分层（账号与服务/对话与风格/系统与数据）
- 界面毛玻璃新拟态改造（玻璃化表面 + 环境光背景，深浅双主题）
- 本机 secrets（DPAPI）与本地配置从提交排除，杜绝凭据入库

---

## 待发布（慕）

- 项目正式更名为“慕”，并统一网页、引导、文档与便携包品牌
- 外部账号改为直接选择角色；联系人与回复令牌继续在内部隔离
- 角色页新增“记忆与内心”，可查看她记得的事和第一人称心事日记

---

## 2026-07-20（v4.3.0）

- 网页端加入停止生成、重新生成、复制、友好错误、关于信息和完整备份/恢复操作
- 备份使用 SQLite 一致性快照；恢复只在下次启动、数据库尚未打开时执行，并自动保留恢复前备份
- Persona 使用 Pydantic 验证与原子保存，人设表达与 Mood 通过明确优先级融合
- ChatPipeline 拆分消息富化、Prompt 上下文组装和后台后处理，后台任务支持有界管理与优雅关闭
- LiteLLM 接入原生 function calling 与流式工具调用，同时保留旧格式兼容
- 记忆检索加入最低语义相似度阈值；新增健康检查、schema migration、上传限制、CI 与覆盖率
